...
class TrainService:
    ...
    async def get_train_by_number(self, train_number: str) -> TrainResponse | None:
        ...
        Returns:
            TrainResponse or None if not found.
        """
        train = await self.repository.get_by_train_number(train_number)
        if not train:
            return None
        return self._train_to_response(train)
    ...
    async def list_trains(
        ...
        if train_type:
            ...
            trains = await self.repository.get_all_with_route(skip=skip, limit=size)

        total = await self.repository.count()

        return TrainListResponse(
            items=[self._train_to_response(t) for t in trains],
            total=total,
            page=page,
            size=size,
