"""SQLAlchemy implementation of the schedule repository for raildbsetup.
...
class SqlScheduleRepository(ScheduleRepository):
            ...
            await self._s.execute(
                sa.text("""
                    INSERT INTO schedules (
                        ...
                        :dow, :platform, :sequence, :distance
                    )
                    """),
                {
                    "train_id": train_id,
    ...
    async def _resolve_station_id(
        ...
        if len(exact_ids) > 1:
            return await self._nearest_candidate(exact_ids, prev_station_id)
