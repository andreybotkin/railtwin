"""Background updater that computes train trajectories and caches them in Redis."""
...
class PositionCacheUpdater:
    def __init__(
        ...
        self._trajectory_interval_seconds = max(interval_seconds, trajectory_interval_seconds)
        ...
        while True:
            ...
            try:
                refresh_trajectories = (
                    self._last_trajectory_refresh == 0.0
                    or (tick_start - self._last_trajectory_refresh) >= self._trajectory_interval_seconds
                )
                static_map_payloads: tuple[dict[str, Any], tuple[list[dict[str, Any]], int]] | None = None
                trajectories: list[dict[str, Any]] = []
                stop_sequences: dict[int, list[dict[str, Any]]] = {}

                async with self._session_factory() as session:
                    simulation_service = TrainSimulationService(session, redis_client=self._redis)
                    _, trajectories, stop_sequences = await simulation_service.get_all_active_train_data(
                ...
                if topology_version is not None and topology_version != self._static_map_topology_version:
                    static_map_payloads = (
                        {
                ...
                active_trains = len(trajectories)
                if refresh_trajectories:
                    self._last_trajectory_refresh = tick_start

                for trajectory in trajectories:
                    trajectory.setdefault("meta", {})["topology_version"] = topology_payload.get("topology_version")

                pipe = self._redis.pipeline()
                pipe.setex(REDIS_TRAJECTORIES_KEY, REDIS_TRAJECTORIES_TTL, json.dumps(trajectories, default=str))
                ...
                if static_map_payloads is not None:
                    ...
                    pipe.set(REDIS_MAP_NETWORK_EDGES_KEY, json.dumps(network_edges_payload, default=str))
                    pipe.set(REDIS_MAP_STATIONS_KEY, json.dumps(stations_payload[0], default=str))
                await pipe.execute()
...
def build_position_cache_updater(session_factory: async_sessionmaker, redis_client: Redis) -> PositionCacheUpdater:
