...
class RouteService:
    ...
    def _payload_to_response(self, route: dict[str, object]) -> RouteResponse:
        ...
        return RouteResponse(
            id=route["id"],
            name=route["name"],
            name_th=route.get("name_th"),
            route_type=route["route_type"],
            distance_km=route.get("distance_km"),
            color=route.get("color"),
            line_geometry=(
                GeoJSONLineString.model_validate(line_geometry)
                if line_geometry
                else None
            ),
            stations=[
                RouteStationInfo.model_validate(station)
    ...
    async def get_route(self, route_id: int) -> RouteResponse | None:
        ...
        """
        route = await self.reader.get_route(route_id)
        if not route:
            return None
        return self._payload_to_response(route)
    ...
    async def list_routes(
        ...
        """
        routes, total = await self.reader.list_routes(
            page=page,
            size=size,
        ...
        return RouteListResponse(
            items=[self._payload_to_response(r) for r in routes],
            total=total,
            page=page,
            size=size,
