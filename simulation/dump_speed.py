...
async def main():
    ...
    if traj:
        print(f"Current segment progress: {traj.meta.segment_progress_pct}%")
        print(f"Current route progress: {traj.meta.route_progress_pct}%")
        print(f"Frames:")
        for i, f in enumerate(traj.frames[:5]):
            print(f"  Frame {i}: speed {f.speed_kmh} km/h, status {f.status}")
    else:
