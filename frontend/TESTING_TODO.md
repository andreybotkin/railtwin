...
- [ ] Test `useTrainTrajectories` hook: TrajectoryWebSocketClient update/delete handling
- [ ] Test `getVehiclePosition()` / `interpolateLineString()` in `trajectory-interpolation.ts`
...
- [ ] Trajectory test: mock /ws/trajectory, verify canvas draws train with heading arrow
- [ ] Selected train: click train marker, verify trajectory polyline overlay appears
- [ ] Smooth motion: record canvas frame sequence, verify position changes are continuous
...
- [ ] Canvas rAF performance: verify 60fps with 200 trains (Chrome DevTools Performance tab)
