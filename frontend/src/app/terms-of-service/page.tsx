...
const TERMS_SECTION_KEYS = [
  ...
  'intellectualPropertyAndDataRights',
...
export default async function TermsOfServicePage() {
  ...
  const termsSections = TERMS_SECTION_KEYS.map((key) => ({
    key,
    title: t(`termsOfServicePage.sections.${key}.title`),
    body: t(`termsOfServicePage.sections.${key}.body`),
  }));
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">{t('termsOfServicePage.title')}</h1>
              <p className="mt-3 text-sm leading-6" style={{ color: 'var(--panel-subtext)' }}>
                {t('legal.lastUpdated')}. {t('termsOfServicePage.description')}
              </p>
            </div>
          </div>
        </section>
