...
Logic is adapted from the original raildatacollector/scripts/load_kml_data.py.
"""
...
def parse_kml_bytes(kml_bytes: bytes) -> tuple[list[RouteData], list[StationData]]:
    ...
    # Placemarks directly under Document
...
def parse_kml_file(path: Path) -> tuple[list[RouteData], list[StationData]]:
    """Parse a local KML file and return domain entities."""
    return parse_kml_bytes(path.read_bytes())


def _pm_name(pm: ET.Element) -> str:
    el = pm.find(_tag("name"))
    return el.text.strip() if el is not None and el.text else "Unnamed"
